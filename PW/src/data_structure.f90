...
SUBROUTINE data_structure( gamma_only )
  ...
  USE symm_base,  ONLY : fft_fact, s, nsym
  ...
  CALL fft_type_init( dffts, smap, "wave", gamma_only, lpara, intra_bgrp_comm, &
       at, bg, gkcut, gcutms/gkcut, fft_fact=fft_fact, s=s, nsym=nsym, nyfft=nyfft, nmany=nmany_, use_pd=pencil_decomposition_  )
  CALL fft_type_init( dfftp, smap, "rho" , gamma_only, lpara, intra_bgrp_comm, &
       at, bg, gcutm , 4.d0, fft_fact=fft_fact, s=s, nsym=nsym, nyfft=nyfft, nmany=nmany_, use_pd=pencil_decomposition_ )
